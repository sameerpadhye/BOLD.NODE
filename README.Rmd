...
This function lets users search by more than 10 different search parameters including taxonomy.names (species, genus, family etc.), geography (country.ocean, province, region), ids (processid, sampleid) and more. The users can just input the search query directly (e.g. put *Canada* as a query in the `geography.names` argument or *Coleoptera* in the `taxonomy.names` argument) with the function searching the query in the relevant fields internally. The search result is a `tbl_sql` object that is used for collecting or transforming the data.
